...
discipline-enforced invariants** introduced *by* those refactors, plus a few files that
...
console commands *should* flow through the watcher), which proves it's a per-callsite
...
nearly-identical "did anything move?" functions whose *difference* is the load-bearing
...
push is a *convention*, copy-pasted at the tail of `selectRow` (`repo-store.tsx:375`),
...
helper that updates the slice *and* pushes to the controller, so the invariant has a
single home.
...
(`:614,:816,:828,:970,:985` and `useRepoEvents`); any *new* reload caller that doesn't
...
(`ipc-contract.ts:56-227`, canonical), `Bridge` (`ipc-contract.ts:249-336`, *derived* —
...
compile time, but a *return-type* drift in the `RepoApi` interface is caught by
...
`preload/index.ts:22` — a typo is a compile error). The six *event* sends in
...
- **`WtAct` round-trip** — `worktree-panel.tsx:47-50` defines actions as closures over
  `RepoApi` (`STAGE = (a, p) => a.stage(p)`), passes them *into* the store, and
...
### 1. `repo-store.tsx` (1181 lines) — trending god-*store*
...
Slices reach into each other freely — `selectRow` writes `selection` *and* `ui`
...
every feature couples through. It is cohesive *today* because one author holds the
...
### 2. `detail-panel.tsx` (672 lines) — a god *file*, not a god component
...
(correctly *not* in the per-repo store), but three extractable hooks are sitting in
...
The through-line: the store refactor and the two prior audits traded *visible*
structural debt (one giant component, broken refresh paths) for *invisible* contract
