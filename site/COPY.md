...
## Section 1 — The graph is the point

Intro: `Most Git clients treat history as a side panel. Amont is built the other way
around: the graph is the workspace, so it has to stay fast and stay legible.`
...
Intro: `GUIs lose your trust the first time they do something you didn't ask for.
...
- A. `Git history you can actually read.` — readability-first; mirrors verbatim
  customer language ("the graph is actually readable"). **Recommended.**
- B. `A Git client that keeps up.` — fluidity-first, punchier, less specific.
- C. `Your whole history, one readable graph.` — descriptive, safest, flattest.
