...
import { api, type FileChange, type Worktree, type WtSource } from "@/lib/git"
...
type Section = {
  ...
  hint?: string
  all?: string
  act?(paths: string[]): Promise<void>
}

const SECTIONS: Section[] = [
  { key: "conflicts", title: "Conflits", source: "unstaged" },
  { key: "staged", title: "Indexés", source: "staged", icon: MinusSignIcon, hint: "Désindexer", all: "Tout désindexer", act: (p) => api.unstage(p) },
  { key: "unstaged", title: "Modifiés", source: "unstaged", icon: PlusSignIcon, hint: "Indexer", all: "Tout indexer", act: (p) => api.stage(p) },
  { key: "untracked", title: "Non suivis", source: "untracked", icon: PlusSignIcon, hint: "Indexer", all: "Tout indexer", act: (p) => api.stage(p) },
]
...
type Props = {
  ...
  onRun(act: (paths: string[]) => Promise<void>, paths: string[]): void
