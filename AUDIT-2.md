...
Trois des findings les plus graves sont la même erreur sous trois formes — *un cache/état qui
survit au remplacement de son état source* :
...
- **Répro** : renderer compromis (modèle de menace du projet) bouclant `window.amont.files(id, "ZZZ", null, <requestId unique>)`. Chaque appel : `assertHash("ZZZ")` throw `BAD_ARG` → un `AbortController` reste en Map → croissance sans borne = DoS mémoire du main. *N'affecte pas l'usage normal (hashs valides).*
